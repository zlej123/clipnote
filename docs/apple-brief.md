...
| 레포 | 역할 | 상태 |
|------|------|------|
| [stepkeeper](https://github.com/zlej123/stepkeeper) | Python 코어 + `src/stepkeeper/skill-core/` 언어중립 자산 | 완료 (테스트 6도메인 54영상 통과) |
...
- 후보 시각: `candidate_times` — before = step.t_start-1, center = best_visual_timestamp, after = step.t_end+1 (영상 범위로 클램프). 원본: `stepkeeper/src/stepkeeper/capture.py`, JS 포팅: `stepkeeper-extension/content.js`.
