"""Validate the existing how-to timeline and visual-guide contract."""

AMBIGUITY_TYPES = {
    "size", "thickness", "color", "state", "amount",
...
def validate_visual_guides(data: dict):
    errors, warnings = [], []
    ...
    for index, step in enumerate(steps):
        ...
        if start < previous_start:
            warnings.append(
                f"{tag}: 시작 시간이 이전 단계보다 앞섬 ({start}<{previous_start})")
    ...
    for index, guide in enumerate(guides):
        ...
        if timestamp is None:
            ...
            errors.append(
                f"{tag}: best_visual_timestamp 범위 밖 ({timestamp}/{duration})")
